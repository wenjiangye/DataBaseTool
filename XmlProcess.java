...
public class XmlProcess {
   ...
   public void InitProcessInfo()           //初始化类运行时信息
   {
      ...
      messRet = null;
      try {
         Class.forName(pro_config.sValDriveName);          //加载JDBC
      }
      catch (ClassNotFoundException e)
      {
         //ShowError("加载JDBC失败..." + "\n驱动名称：" + pro_config.sValDriveName);
         e.printStackTrace();
      }
   }
  ...
  private void SetConnectInfo()
   ...
   public boolean  ExecuteSQL(String m_Sql)
   {
         ...
         try
         {
            ...
            if (!m_noShow)
            {
               ...
               if(messRet != null)
                  messRet.callback(User_ID, m_Sql);
            }
            stConnectINfo.isOpenResult = false;
   ...
   public void ShowError(String mess)
   {
      ...
      if (stXmlRunInfo.cCurrentSqlCase.stCaseResult.bExpResult)
      {
         ...
         if(messRet != null)
            messRet.callback(User_ID, "失败:" + mess + "\n");
      }
      else
      {
         frame.AppendMess("成功消息:" +  mess,true);
         if(messRet != null)
            messRet.callback(User_ID, "成功消息:" + mess + "\n");
      }
   }
